...
def print_s2e_cmd():
    s2e_cmd = """
以下是c程序和汇编程序从源码变成可执行文件的简介

############################################################## c overview ########################################################################

源代码 → 预处理 → 编译成汇编 → 汇编成机器码 → 链接成可执行文件。
...
作用：
    把多个目标文件和库文件（如 libc）连接在一起
    解析函数调用（如 printf）
    确定各段在内存中的地址
    生成最终的 可执行文件（ELF）
输出结果：可执行文件（hello），可以由操作系统加载运行

############################################################ ass overview ########################################################################

汇编程序从源码到可执行文件的过程，也分为 汇编 → 链接 → 执行 三个主要阶段，比C语言少了“预处理”和“编译”两个步骤。

1. 汇编（Assembling）
命令示例： as hello.s -o hello.o 或者 gcc -c hello.s -o hello.o
作用：
    汇编器（as）把汇编源码（.s）翻译成 机器指令。
    同时生成 目标文件（.o），其中包含：
        二进制机器码（暂未确定最终地址）
        符号表（如 _start、printf）
        各段信息（.text、.data、.bss）
...
2. 链接（Linking）
命令示例： ld hello.o -o hello 或者 gcc hello.o -o hello
作用：
    符号解析：找到各符号（如函数名、变量名）的实际地址。
    地址重定位：把代码和数据段放到最终的虚拟地址空间中。
    合并段信息：将 .text、.data 等合并成最终结构。
    最终生成 可执行文件（ELF格式）。
	
3. 云行（Execution）
操作系统执行：
    1、加载器（Loader） 读取 ELF 文件头；
    2、把程序段映射到内存；
    3、从 入口地址（Entry Point）（通常是 _start）开始执行；
    4、程序运行，直到 exit 系统调用结束。
   """
    print(s2e_cmd) 
