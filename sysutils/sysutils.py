...
2、MBR 格式单个磁盘最大支持到2T的存储空间，即分区最大也只支持2T 的存储空间
   """
    print(lvm_cmd) 
...
wget 是 Linux 下非常常用的命令行下载工具，功能强大，支持断点续传、递归下载、代理、限速等功能。
...
| ------------------------------------- | ---------------------------- 
| `-O <文件名>`                         | 将下载内容保存为指定文件名
| `-o <logfile>`                        | 调试日志（查看详细错误）
| `-c`                                  | **断点续传**（续传未完成的下载）
| `-q`                                  | 安静模式（不输出信息）
| `-nv`                                 | 简洁输出模式（比默认少，但比 `-q` 多）
| `-t <次数>`                           | 失败重试次数（默认是 20）
| `--retry-connrefused`                 | 如果连接被拒绝也重试
| `--timeout=<秒>`                      | 设置连接/读取的超时时间
| `--wait=<秒>`                         | 每次下载之间等待多少秒（防止被封 IP）
| `--limit-rate=<速度>`                 | 限制下载速度，例如 `200k` 或 `2m`
| `--user=<用户名>` `--password=<密码>` | 支持需要认证的网站
| `--user-agent=<UA>`                   | 自定义 User-Agent（伪装成浏览器）
...
| -------------------- | ---------------------------------------------
| `-r` 或 `--recursive`| 递归下载
| `-l <级别>`          | 最大递归深度（默认 5，0 表示无限）
| `--no-parent`        | 不下载父目录的内容
| `--convert-links`    | 下载后转换页面中的链接
| `--mirror`           | 镜像整个网站，相当于 `-r -N -l inf --no-remove-listing`
| `-np`                | 同 `--no-parent`，避免上层目录抓取
...
| `--proxy=on/off`                                 | 开启或关闭代理支持
| `--proxy-user=<用户名>` `--proxy-password=<密码>`| 使用代理认证
...
| `-N`           | 只下载更新过的文件（基于时间戳）
| `-P <目录>`    | 设置下载文件的保存目录
| `--spider`     | 模拟下载，不保存文件（可用于检测链接）
| `-i <文件>`    | 从文件中读取 URL 批量下载
...
# 调试日志（查看详细错误）：
wget -o debug.log <下载链接>

# 使用 --user-agent 模拟浏览器访问：
wget --user-agent="Mozilla/5.0" <下载链接>
...
def print_psstat_cmd():
    psstat_cmd = """
############################################################## 基本状态码 ########################################################################

基本状态码（单字母）

R (Running)：正在运行，或者在运行队列中等待 CPU 时间片。
S (Sleeping)：可中断的睡眠状态（等待某个事件，比如 I/O 完成）。
D (Uninterruptible sleep)：不可中断的睡眠（通常是等待磁盘或网络 I/O，不能用 kill -9 杀掉）。
T (Stopped)：暂停状态，可能是收到 SIGSTOP 信号，或者在调试时被 ptrace 停止。
t (tracing stop)：被调试器追踪时暂停。
Z (Zombie)：僵尸进程，表示进程已经退出，但父进程还没有调用 wait() 回收它的退出状态。
X (Dead)：几乎不会看到，表示进程已经彻底消失。


| **状态码**| **含义**                               | **常见场景 / 示例进程**
|  -------- | -------------------------------------- | ------------------------------------
|   **R**   | Running，运行中或在运行队列中          | `top`、`yes`、CPU 密集任务
|   **S**   | Sleeping，可中断睡眠（等待事件）       | `bash` 等待输入、`sshd` 等待连接
|   **D**   | Uninterruptible sleep，不可中断睡眠    | `dd if=/dev/sda of=/dev/null` 进行磁盘拷贝
|   **T**   | Stopped，被信号暂停（如 `SIGSTOP`）    | `kill -STOP pid` 后的进程
|   **t**   | Tracing stop，被调试器追踪时暂停       | `gdb` attach 的目标进程
|   **Z**   | Zombie，僵尸进程，退出但未被父进程回收 | `some_app <defunct>`
|   **X**   | Dead，进程已消亡（很少见）             | 内核中偶尔可见
...
N：低优先级进程（nice 值大）。
L：进程有锁定的内存页，常见于实时进程。
s：该进程是一个会话首领（session leader）。
l：多线程进程（使用 CLONE_THREAD，比如 pthread）。
+：该进程位于前台进程组（foreground）。


| **修饰符**| **含义**                        | **示例**
|  -------- | ------------------------------- | --------------------------
|   **<**   | 高优先级（nice 值小）           | `migration/0` 内核调度线程（`R<`）
|   **N**   | 低优先级（nice 值大）           | `updatedb` 后台索引任务（`SN`）
|   **L**   | 有锁定内存页（real-time/MLock） | 一些实时进程
|   **s**   | 会话首领（session leader）      | 登录 shell（`Ss`）
|   **l**   | 多线程（CLONE\_THREAD）         | `java`、`nginx`（`Sl`）
|   **+**   | 前台进程组                      | 你终端里运行的命令（`R+`、`S+`）

############################################################## instance ########################################################################

R+ → 正在运行，并且是前台进程。
Sl → 可中断睡眠 + 多线程（Java 程序一般是多线程）。
Ss → 会话首领 + 睡眠状态（典型是守护进程）。
Z → 僵尸进程（<defunct>）。
D：不可中断的睡眠，说明它正在等待磁盘 I/O 完成。
R<：运行状态（R），优先级高（<），典型的内核线程。
SN：睡眠状态（S），低优先级（N）。

   """
    print(psstat_cmd) 

def print_rootdir_cmd():
    rootdir_cmd = """
####################################################### 根目录 / 下常见目录简要说明 ################################################################

/bin：基础命令（如 ls、cp），系统最小运行必需
/sbin：系统管理命令，主要给 root 使用
/boot：启动文件（内核、grub），删错会无法启动
/etc：系统和服务的配置文件
/dev：设备文件（磁盘、终端等）
/proc：虚拟目录，反映内核和进程状态
/sys：虚拟目录，描述硬件和驱动信息
/usr：应用程序和库（类似“程序安装目录”）
/var：经常变化的数据（日志、缓存、队列）
/home：普通用户家目录
/root：root 用户家目录
/tmp：临时文件目录（重启可能清空）
/mnt / /media：挂载点（临时/自动挂载）
...
| `/sys/class`          | 功能分类
| `/sys/devices`        | 硬件拓扑
| `/sys/devices/system` | CPU / NUMA
| `/sys/firmware`       | BIOS / UEFI
| `/sys/fs`             | 内核子系统
| `/sys/kernel`         | 内核参数
| `/sys/module`         | 内核模块
| `/sys/power`          | 电源管理
...
/sys 是虚拟文件系统（sysfs）
内核 2.6 引入
用来反映：设备、驱动、总线、内核对象之间的关系

/proc 是虚拟文件系统（procfs）
不占用磁盘空间
内容由内核实时生成
用来向用户态暴露：进程信息、系统运行状态、内核参数
...
def print_sec_enh_cmd():
    sec_enh_cmd = """

########################################################## 可以登录系统的用户 #####################################################################

awk -F: '$7 !~ /nologin|false|sync|shutdown|halt/' /etc/passwd
...
# /etc/passwd 结果中所有行的第二个冒号后的ID数值均不相同
awk -F: '{users[$3] = users[$3] " " $1} END {for (uid in users) {if (split(users[uid], arr) > 1) print "UID " uid ": " users[uid]}}' /etc/passwd

####################################################### 禁止root权限用户直接远程登录 ###############################################################

1. 禁止root用户远程ssh登录：
修改/etc/ssh/sshd_config文件,配置PermitRootLogin no。重启服务，systemctl restart sshd
...
1、pam_pwquality.so 模块的生效关联
/etc/pam.d/system-auth和/etc/pam.d/password-auth两个文件中配置了该模块的调用，格式如下：
password    requisite     pam_pwquality.so try_first_pass retry=3


2、pwquality.conf 核心配置项（常用）
minlen = 9          # 密码最小长度，默认9
dcredit = -1         # 至少包含1个数字（-n表示至少n个，n表示最多n个）
ucredit = -1         # 至少包含1个大写字母
lcredit = -1         # 至少包含1个小写字母
ocredit = -1         # 至少包含1个特殊字符（!@#$%^&*等）
minclass = 4         # 至少包含4类字符（数字、大写、小写、特殊字符）
maxrepeat = 3        # 同一字符连续出现的最大次数，默认3
maxsequence = 4      # 连续字符序列的最大长度（如1234、abcd）
reject_username     # 禁止密码包含用户名本身
maxdiff = 3          # 新密码与旧密码的最少不同字符数

############################################################## 口令生存期 ########################################################################

/etc/login.defs
login.defs 既不属于 PAM 模块，也不由任何 PAM 模块定义，它是 shadow-utils 软件包（包含useradd、passwd、chage等工具）的默认配置文件

PASS_MAX_DAYS   90    # 密码最长有效期（天）
PASS_MIN_DAYS   1     # 密码修改后，最短多久可再次修改（天）
PASS_WARN_AGE   7     # 密码到期前，提前多少天提醒用户（天）
UID_MIN                  1000    # 普通用户最小UID
GID_MIN                  1000    # 普通用户最小GID

注意：login.defs的配置仅对新创建的用户生效，对已存在的用户需用chage命令修改（如chage -M 90 root）。

########################################################## 命令行界面超时退出 #####################################################################

vi /etc/profile
增加 export TMOUT=600(单位：秒，可根据具体情况设定超时退出时间，要求不小于600秒)

############################################################ 最大并发连接数 #######################################################################

vim /etc/security/limits.conf
# 若需对除了nginx/mysql等用户的所有用户进行限制，使用 * 为用户名即可
