...
if [ ! -x "backup" ]; then
    ...
    if [ -f /etc/pam.d/system-auth ];then
        \cp -f /etc/pam.d/system-auth bash/backup/system-auth.bak
    elif [ -f /etc/pam.d/common-password ];then
        \cp -f /etc/pam.d/common-password bash/backup/common-password.bak
    fi
    if [ -f ~/.ssh/authorized_keys ];then
        \cp -f ~/.ssh/authorized_keys bash/backup/authorized_keys.bak
    fi
    \cp -f /etc/pam.d/sshd bash/backup/sshd.bak
    \cp -f /etc/sudoers bash/backup/sudoers.bak
    \cp -f /etc/ssh/sshd_config bash/backup/sshd_config.bak
    \cp -f /etc/profile bash/backup/profile.bak
    \cp -f /etc/pam.d/su bash/backup/su.bak
    echo -e "###########################################################################################"
...
###########################文件还原############################
function recover(){
if [ -f bash/backup/system-auth.bak ];then
    \cp -rf bash/backup/system-auth.bak /etc/pam.d/system-auth
elif [ -f bash/backup/common-password.bak ];then
    \cp -rf bash/backup/common-password.bak /etc/pam.d/common-password
fi
if [ -f bash/backup/authorized_keys.bak ];then
    \cp -rf bash/backup/authorized_keys.bak ~/.ssh/authorized_keys
fi
    \cp -rf bash/backup/sshd.bak /etc/pam.d/sshd
    \cp -rf bash/backup/sudoers.bak /etc/sudoers
    \cp -rf bash/backup/sshd_config.bak /etc/ssh/sshd_config
    \cp -rf bash/backup/profile.bak /etc/profile
    source /etc/profile
    \cp -rf bash/backup/su.bak /etc/pam.d/su
    restart_flag=0
